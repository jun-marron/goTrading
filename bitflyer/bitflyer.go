...
type RseponseSendChildOrder struct {
	ChildOrderAcceptanceID string `json:"child_order_acceptance_id"`
}

func (api *APIClient) SendOrder(order *Order) (*RseponseSendChildOrder, error) {
	...
	var response RseponseSendChildOrder
...
func (api *APIClient) ListOrder(query map[string]string) ([]Order, error) {
	resp, err := api.doRequest("GET", "me/getchildoders", query, nil)
	if err != nil {
		return nil, err
	}
