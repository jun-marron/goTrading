...
func (df *DataFrameCandle) AddMacd(inFastPeriod int, inSlowPeriod int, inSignalPeriod int) bool {
	if len(df.Candles) > 1 {
...
func (df *DataFrameCandle) BackTestEma(period1, period2 int) *SignalEvents {
	...
	for i := 1; i < lenCandles; i++ {
		...
		if emaValue1[i-1] < emaValue2[i-1] && emaValue1[i] >= emaValue2[i] {
			signalEvents.Buy(df.ProductCode, df.Candles[i].Time, df.Candles[i].Close, 0.0001, false)
		}

		if emaValue1[i-1] > emaValue2[i-1] && emaValue1[i] <= emaValue2[i] {
			signalEvents.Sell(df.ProductCode, df.Candles[i].Time, df.Candles[i].Close, 0.0001, false)
		}
	}
	return signalEvents
}
...
func (df *DataFrameCandle) BackTestBb(n int, k float64) *SignalEvents {
	...
	for i := 1; i < lenCandles; i++ {
		...
		if bbDown[i-1] > df.Candles[i-1].Close && bbDown[i] <= df.Candles[i].Close {
			signalEvents.Buy(df.ProductCode, df.Candles[i].Time, df.Candles[i].Close, 0.001, false)
		}
		if bbUp[i-1] < df.Candles[i-1].Close && bbUp[i] <= df.Candles[i].Close {
			signalEvents.Sell(df.ProductCode, df.Candles[i].Time, df.Candles[i].Close, 0.001, false)
		}
	}
	return signalEvents
}
